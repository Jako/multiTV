...
class multiTV
{
    ...
    function generateScript()
    {
        ...
        if ($this->configuration['enablePaste'] && $this->display != 'dbtable') {
            ...
            $files['scripts'] = array_merge($files['scripts'], $settings['scripts'], json_decode($tvfields ,1)['scripts']);
            $files['css'] = array_merge($files['css'], $settings['css'], json_decode($tvfields ,1)['css']);
